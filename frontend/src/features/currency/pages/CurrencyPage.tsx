...
// 1. Importar o novo serviço refatorado como um objeto
...
const CurrencyPage: React.FC = () => {
  ...
  const fetchCurrencies = async () => {
    try {
        // 2. Chamar a nova função do serviço
  ...
  const handleSave = async (currency: Currency) => {
    ...
    try {
      if (currency.id) {
        // 3. Chamar a nova função de update
        await currencyService.update(currency.id, currency);
      } else {
        // 4. Chamar a nova função de create
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Deseja excluir esta moeda?")) {
      try {
        // 5. Chamar a nova função de delete
